...
In a real app, resolvers import server-only dependencies like
database clients. Use the `server-only` package to make sure
these files never leak into the client bundle — the build will
fail if a client component imports them (even transitively).
...
See [Project structure](/docs/project-structure) for more on
keeping server code out of the client bundle.
